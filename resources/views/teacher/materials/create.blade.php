...
@section('header', 'Tambah Materi Baru')

@section('content')
<div class="max-w-5xl mx-auto pb-32">
    
    <div class="mb-6 flex justify-between items-center">
        <h1 class="text-3xl font-black text-white tracking-tight">Rakit Materi Baru</h1>
        <a href="{{ route('teacher.chapters.show', $chapter->id) }}" class="px-5 py-2.5 rounded-xl border border-slate-600 text-slate-300 text-sm font-bold hover:bg-slate-700 transition-colors flex items-center gap-2">
            <span>←</span> Batal & Kembali ke Bab {{ $chapter->sequence }}
        </a>
    </div>

    <div class="mb-8 bg-blue-500/10 border-l-4 border-blue-500 p-5 rounded-r-2xl shadow-sm">
        <h3 class="text-blue-400 font-bold flex items-center gap-2 text-lg">
            <span>✨</span> Material Block Builder Pro
        </h3>
        <p class="text-sm text-slate-300 mt-1">
            Susun materi dengan mudah seperti menyusun balok lego. Sistem akan otomatis merapikan desainnya menjadi tampilan vertikal yang elegan untuk siswa!
        </p>
    </div>
    ...
    <div class="bg-slate-800 rounded-3xl p-8 shadow-xl border border-slate-700 relative overflow-hidden">
        ...
        <form action="{{ route('teacher.materials.store', $chapter->id) }}" method="POST" class="space-y-8 relative z-10" id="materialForm" enctype="multipart/form-data">
            @csrf
            ...
            <div class="p-6 bg-slate-900/50 rounded-2xl border border-slate-700">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Urutan Materi di Bab Ini <span class="text-red-500">*</span></label>
                        <input type="number" name="sequence" required placeholder="Contoh: 1" class="w-full px-5 py-3.5 rounded-xl border border-slate-600 bg-slate-900 text-white focus:ring-2 focus:ring-eduPrimary focus:border-eduPrimary transition-all">
                    </div>
                    <div>
                        <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Minimal Level (XP)</label>
                        <input type="number" name="min_level" required placeholder="Contoh: 1" value="1" class="w-full px-5 py-3.5 rounded-xl border border-slate-600 bg-slate-900 text-white focus:ring-2 focus:ring-eduPrimary focus:border-eduPrimary transition-all">
                    </div>
                </div>

                <div>
                    <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2">Judul Materi Pembelajaran <span class="text-red-500">*</span></label>
                    <input type="text" name="title" required placeholder="Contoh: Pengenalan Algoritma K-Means" class="w-full px-5 py-4 rounded-xl border border-slate-600 bg-slate-900 text-white focus:ring-2 focus:ring-eduPrimary focus:border-eduPrimary transition-all font-black text-lg">
                </div>
            </div>
            ...
            <div>
                ...
                <div id="content-builder" class="space-y-5 mb-8">
                    {{-- Blok akan di-inject di sini oleh JS --}}
                </div>
                ...
                <div class="bg-slate-900 rounded-2xl border-2 border-dashed border-slate-600 p-8 text-center hover:border-eduPrimary/50 transition-colors">
                    <p class="text-xs font-bold text-slate-400 mb-6 uppercase tracking-widest">Pilih Komponen Blok Selanjutnya</p>
                    
                    <div class="flex flex-wrap gap-4 justify-center">
                        <button type="button" onclick="addBlock('title')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-blue-500 hover:bg-blue-500/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">📌</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-blue-400">Judul (H3)</span>
                        </button>
                        
                        <button type="button" onclick="addBlock('subtitle')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-blue-400 hover:bg-blue-400/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">🔖</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-blue-300">Sub-Judul</span>
                        </button>

                        <button type="button" onclick="addBlock('text')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-emerald-500 hover:bg-emerald-500/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">📝</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-emerald-400">Paragraf</span>
                        </button>

                        <button type="button" onclick="addBlock('list')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-emerald-400 hover:bg-emerald-400/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">📋</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-emerald-300">Poin/List</span>
                        </button>

                        <div class="w-px bg-slate-700 mx-2"></div>

                        <button type="button" onclick="addBlock('image')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-purple-500 hover:bg-purple-500/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">🖼️</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-purple-400">Gambar</span>
                        </button>

                        <button type="button" onclick="addBlock('youtube')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-red-500 hover:bg-red-500/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">📺</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-red-400">YouTube</span>
                        </button>

                        <button type="button" onclick="addBlock('alert')" class="flex flex-col items-center justify-center p-4 w-28 bg-slate-800 border border-slate-600 rounded-xl hover:border-amber-500 hover:bg-amber-500/10 transition-colors group">
                            <span class="text-2xl mb-2 group-hover:scale-110 transition-transform">💡</span>
                            <span class="text-xs font-bold text-slate-300 group-hover:text-amber-400">Kotak Info</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="fixed bottom-0 left-0 md:left-[260px] lg:left-[280px] right-0 bg-[#0f172a]/90 backdrop-blur-xl border-t border-slate-700 p-4 md:px-8 z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] flex justify-between items-center">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold text-white">Siap Dirakit</p>
                    </div>
                </div>
...
<script>
    ...
    window.onload = () => { addBlock('title'); addBlock('text'); };
</script>
