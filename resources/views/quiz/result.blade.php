...
<div class="max-w-3xl mx-auto text-center pt-8 px-4 pb-20">
    ...
    <p class="text-base text-slate-400 mb-8 max-w-lg mx-auto leading-relaxed">
        ...
        @elseif($passed)
            Anda berhasil menyelesaikan evaluasi ini dengan hasil yang memuaskan.
        @else
            Jangan menyerah. Pelajari kembali materi sebelumnya dan coba ulangi evaluasi ini.
        @endif
    </p>
        ...
        <div class="relative z-10">
            ...
            <div class="flex justify-center gap-6 text-sm font-bold text-slate-300">
                ...
                <span class="flex items-center gap-2 bg-red-500/10 px-4 py-1.5 rounded-lg text-red-400 border border-red-500/20">
                    ✕ {{ $totalQuestions - $correctCount }} Jawaban Salah
                </span>
            </div>
        </div>
        ...
        <div class="space-y-6">
            @foreach($quiz->questions as $index => $question)
                @php
                    ...
                    $isCorrect = strtolower($myAnswer) == strtolower($question->correct_answer);
    ...
    <div class="flex flex-col sm:flex-row gap-4 justify-center mt-12">
        <a href="{{ route('dashboard') }}" class="px-8 py-3.5 rounded-xl border border-slate-600 bg-slate-800 text-slate-300 font-bold hover:bg-slate-700 hover:text-white transition-all">
            Kembali ke Dashboard
        </a>

        @if($quiz->type == 'pre_test')
            {{-- TOMBOL KHUSUS PRE-TEST (LANJUT MATERI) --}}
            @if($quiz->chapter)
                 <a href="{{ route('learning.show', $quiz->chapter->materials->first()->slug ?? '#') }}" class="px-8 py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-lg shadow-emerald-500/20 transition-all">
                    Mulai Belajar Materi Bab 1 ➔
                </a>
            @endif
        @else
            {{-- TOMBOL UNTUK EVALUASI / POST-TEST --}}
            @if(!$passed)
                <a href="{{ route('quiz.show', $quiz->id) }}" class="px-8 py-3.5 rounded-xl bg-eduPrimary hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
                    Ulangi Evaluasi
                </a>
            @elseif($passed && $quiz->chapter)
                 <a href="{{ route('learning.show', $quiz->chapter->materials->first()->slug ?? '#') }}" class="px-8 py-3.5 rounded-xl bg-eduPrimary hover:bg-blue-700 text-white font-bold shadow-lg shadow-blue-500/20 transition-all">
                    Materi Selanjutnya ➔
                </a>
            @endif
        @endif
